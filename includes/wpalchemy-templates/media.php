...
<div class="mm_admin_box mm_admin_media">
	<p>The media section allows you to upload or reference audio and video to be displayed along side the message content above. It can be displayed on the sermon page, using shortcodes, and in podcasts.</p>
	<h4>Video</h4>
	...
	<tbody>
		...
		<tr id="mm-video-type">
			...
			<td>
				<select id="mm-video-type-select" name="<?php $metabox->the_name(); ?>">
		...
		<?php $wpalchemy_media_access->setGroupName('video-url-n')->setInsertButtonLabel('Use File'); ?>			
		<tr id="mm-video-url">
			<th scope="row"><label>URL:</label></th>
			<td>
			<?php echo $wpalchemy_media_access->getField(array('name' => $mb->get_the_name(), 'value' => $mb->get_the_value())); ?>
			<?php echo $wpalchemy_media_access->getButton(array('label' => 'Upload/Select Video')); ?>
	...
	<tbody>
		...
		<?php $wpalchemy_media_access->setGroupName('audio-url-n')->setInsertButtonLabel('Use File'); ?>			
		<tr>
			<th scope="row"><label>URL:</label></th>
			<td>
			<?php echo $wpalchemy_media_access->getField(array('name' => $mb->get_the_name(), 'value' => $mb->get_the_value())); ?>
			<?php echo $wpalchemy_media_access->getButton(array('label' => 'Upload/Select Audio')); ?>
			<p class="description">Enter the URL to an audio file, upload a file from your computer, or choose a file already in the media library.</p>
			</td>
		</tr>
	...
	jQuery(document).ready(function($) {
		...
		function toggleFields() {
			...
			if (type == 'url') {
				...
				$('.mediabutton-video-url-n').fadeIn('fast');
				...
				$('#mm-video-url').fadeIn('fast');
				$('.mediabutton-video-url-n').fadeOut('fast');
