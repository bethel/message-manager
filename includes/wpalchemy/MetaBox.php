...
add_action('admin_head', array('MMWPAlchemy_MetaBox', '_global_head'));

add_action('admin_footer', array('MMWPAlchemy_MetaBox', '_global_foot'));
...
class MMWPAlchemy_MetaBox
{
	...
	function MMWPAlchemy_MetaBox($arr)
	{
	...
	function _init()
	{
		...
		if ( ! MMWPAlchemy_MetaBox::_is_post() AND ! MMWPAlchemy_MetaBox::_is_page()) return;
	...
	function _is_post()
	{
		if ('post' == MMWPAlchemy_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	function _is_page()
	{
		if ('page' == MMWPAlchemy_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		$post_type = MMWPAlchemy_MetaBox::_get_current_post_type();
	...
	function _get_current_post_type()
	{
		...
		if ( isset( $uri ) )
		{
			...
			{
				$post_id = MMWPAlchemy_MetaBox::_get_post_id();
	...
	function _global_head()
	{
		...
		jQuery(function($)
		{
			...
			{		
				...
				if (elem.attr('class') && elem.filter('[class*=dodelete]').length)
				{
					...
					if (confirm('This action can not be undone, are you sure?'))
					{
						if (the_name)
						{
							$('.wpa_group-'+ the_name, p).not('.tocopy').fadeOut('fast', function() {
								$(this).remove();
						...
						{
							elem.parents('.wpa_group').fadeOut('fast', function() {
								$(this).remove();
								the_name = elem.parents('.wpa_group').attr('class').match(/wpa_group-([a-zA-Z0-9_-]*)/i)[1];
								checkLoopLimit(the_name);
								$.wpalchemy.trigger('wpa_delete');
			...
			$('[class*=docopy-]').click(function(e)
			{
				...
				{
					$('.wpa_group-'+ the_name +':first', p).before($(the_clone).fadeIn('fast', function() {
						checkLoopLimit(the_name);
				...
				{
					the_group.before($(the_clone).fadeIn('fast', function() {
						checkLoopLimit(the_name);
						$.wpalchemy.trigger('wpa_copy', [the_clone]);
					}));
				}
	...
	function _save($post_id) 
	{
		...
		MMWPAlchemy_MetaBox::clean($new_data);
	...
	function clean(&$arr)
	{
		if (is_array($arr))
		{
			foreach ($arr as $i => $v)
			{
				if (is_array($arr[$i])) 
				{
					MMWPAlchemy_MetaBox::clean($arr[$i]);
